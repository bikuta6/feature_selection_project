...
from feature_enhancer.feature_selection.crossover import (
    SinglePointCrossover,
    TwoPointCrossover,
    UniformCrossover,
    ArithmeticCrossover,
)
from feature_enhancer.feature_selection.mutation import (
    RandomBitFlip,
    UniformMutation,
    BlockMutation,
    AdaptiveMutation,
...
def create_model(model_type, task_type, random_state=42):
    ...
    if task_type not in models:
        raise ValueError(
            f"Tipo de tarea no soportado: {task_type}. Use 'regression' o 'classification'"
        )

    if model_type not in models[task_type]:
        raise ValueError(
            f"Tipo de modelo no soportado: {model_type}. Use {list(models[task_type].keys())}"
        )
...
def create_crossover_operator(crossover_type, **kwargs):
    ...
    if crossover_type not in operators:
        raise ValueError(
            f"Tipo de cruzamiento no soportado: {crossover_type}. Use {list(operators.keys())}"
        )
...
def create_mutation_operator(mutation_type, mutation_prob, **kwargs):
    ...
    if mutation_type not in operators:
        raise ValueError(
            f"Tipo de mutación no soportado: {mutation_type}. Use {list(operators.keys())}"
        )
...
def print_results(selector, args):
    ...
    metric = (
        "accuracy"
        if args.metric == "auto"
        and info.get("secondary_objective_type") == "classification"
        else args.metric
    )
    if args.metric == "auto":
    ...
    print(
        f"  {args.secondary_objective.title()}: {info['objectives'][args.secondary_objective]:.4f}"
    )
    ...
    if args.verbose and len(pareto_front) > 1:
        ...
        for i, ind in enumerate(pareto_front):
            ...
            print(
                f"  Solución {i+1}: {n_features} características, "
...
def get_crossover_kwargs(args):
    ...
    """
    return {"swap_prob": args.swap_prob, "alpha": args.alpha}
