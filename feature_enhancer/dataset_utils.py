    ...
    def load_csv(
        ...
        **kwargs
        ...
        if isinstance(target_column, str):
            ...
            raise ValueError("target_column must be string (column name) or int (column index)")
...
def print_dataset_summary(info: dict):
    ...
    if info['categorical_features']:
        print(f"Categorical features ({len(info['categorical_features'])}): {info['categorical_features'][:5]}{'...' if len(info['categorical_features']) > 5 else ''}")
    
    # Missing values
    total_missing_features = sum(v for v in info['missing_values_features'].values() if v > 0)
